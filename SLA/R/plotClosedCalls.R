...
plotClosedCalls = function(cfg, customersData)
{
  ...
  for (iter_customer in customersData[1:length(customersData)]){
    ...
    # Make the plot
    if(cfg$post_process$plot_closed_calls$plot_format == "png"){
      # Name and size of plot
      png(filename = paste0(cfg$folders$postprocessed, 'ClosedCalls_', customer, '.png'),
          width = cfg$post_process$plot_closed_calls$plot_size[1],
          height = cfg$post_process$plot_closed_calls$plot_size[2])
      # Other graphical parameters
      par(mai = rep(cfg$post_process$plot_closed_calls$margin, 4))
      plot(x = Date,
           y = Calls,
           type = cfg$post_process$plot_closed_calls$plot_type,
           main = paste0('Customer: ', customer),
           xlab = cfg$post_process$plot_closed_calls$axes_names[2],
           ylab = cfg$post_process$plot_closed_calls$axes_names[1],
           frame.plot = cfg$post_process$plot_closed_calls$frame_plot,
           pch = cfg$post_process$plot_closed_calls$plot_symbol,
           col = cfg$post_process$plot_closed_calls$color,
           cex.lab = cfg$post_process$plot_closed_calls$label_size,
           cex.main = cfg$post_process$plot_closed_calls$main_size,
           cex = cfg$post_process$plot_closed_calls$text_size,
           cex.axis = cfg$post_process$plot_closed_calls$axis_size,
           #ylim = cfg$post_process$plot_closed_calls$ylim,
           lwd = cfg$post_process$plot_closed_calls$line_width)
      dev.off()
    }
